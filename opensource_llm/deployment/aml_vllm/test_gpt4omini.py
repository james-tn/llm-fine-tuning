...
api_key = os.getenv("AZURE_OPENAI_ENDPOINT_KEY")  # Ensure you have set this in your environment
api_base = os.getenv("AZURE_OPENAI_ENDPOINT")  # Ensure you have set this in your environment
client = AzureOpenAI(
    azure_endpoint=api_base,  # Azure OpenAI endpoint
    api_key=api_key,  # Azure OpenAI API key
    api_version=os.getenv("AZURE_OPENAI_API_VERSION", "2023-05-15")  # Default API version
...
def send_request():  
    ...
    outputs =  client.chat.completions.create( model="gpt-4o-mini", messages=messages
)
    print(outputs)
    end_time = time.time()  
