...
def configure_fsdp_plugin():  
    # Configure the FSDP plugin with your desired settings  
...
def main(args):  
    logging.basicConfig(level=getattr(logging, args.verbosity.upper()))  
        # Load the accelerator with the specific configuration file  
    # Update the YAML configuration with environment variables  
    # Configure the FSDP plugin  
    fsdp_plugin = configure_fsdp_plugin()  
  
    # Initialize the accelerator with the FSDP plugin  
    ...
    logging.info("uses_fsdp %s ", uses_fsdp)
