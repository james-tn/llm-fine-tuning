...
def generate_business_questions_and_queries(schema, prompt_type="single_scenario"):  
    user_message = f"""  
    You are responsible for training new employees on how to effectively utilize our database. Using the provided database schema and business metrics definitions, generate at least 20 diverse and creative business questions, ranging from easy to advanced levels. Each question should be paired with a corresponding SQL query that answers it.  
    ...
    if prompt_type == "cross_scenario":  
        user_message = f"""  
        You are responsible for training new employees on how to effectively utilize our database. Using the provided combined database schema and business metrics definitions from multiple scenarios, generate at least 20 diverse and creative business questions, ranging from easy to advanced levels. Each question should be paired with a corresponding SQL query that answers it.  
  
        ## Combined Database Schema and Business Metrics Definitions  
        {schema}  
...
def main():  
    ...
    with open("../data/all_scenarios_questions_and_queries.json", "w") as file:  
    ...
    train, test = train_test_split(all_data, test_size=0.1, stratify=[item["scenario"] for item in all_data])  
    ...
    with open("../data/sqltest_openai_v2.jsonl", "w") as f:  
        for item in test:  
            schema = json.dumps(split_scenarios[item["scenario"]], indent=4) if item["scenario"] != "cross_scenario" else split_scenarios["cross_scenario"]  
            f.write(json.dumps(create_message_format(item, schema)) + "\n")  
    ...
    print("Train and test data saved to ../data/train_data_v3.json and ../data/test_data_v3.json")  
